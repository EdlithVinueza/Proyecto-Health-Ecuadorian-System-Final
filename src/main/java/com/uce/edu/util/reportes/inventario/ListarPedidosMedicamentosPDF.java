...
/**
 * ListarPedidosMedicamentosPDF
 * 
 */
public class ListarPedidosMedicamentosPDF {
    /**
     * Método que permite generar un PDF con la lista de pedidos de medicamentos
     * 
     * @param listaMedicamentos
     * @return
     * @throws IOException
     */
    public ByteArrayOutputStream generatePdfPedidosMedicamentos(List<PedidoMedicamento> listaMedicamentos)
            throws IOException {
        ...
        try {
            ...
            Paragraph title = new Paragraph("Lista de Pedidos Medicamentos", darkBlueFont);
            ...
            String[] headers = { "Código Pedido", "Proveedor", "Ruc Proveedor", "Medicamento", "Nombre Comercial",
                    "Código Medicamento", "Fecha Pedido", "Estado", "Cantidad", "Total" };
            ...
            for (PedidoMedicamento pedidoMedicamento : listaMedicamentos) {
                table.addCell(pedidoMedicamento.getCodigo());
                table.addCell(pedidoMedicamento.getProveedor().getNombreEmpresa());
                table.addCell(pedidoMedicamento.getProveedor().getRuc());
                table.addCell(pedidoMedicamento.getMedicamento().getNombreComun());
                table.addCell(pedidoMedicamento.getMedicamento().getNombreComun());
                table.addCell(pedidoMedicamento.getMedicamento().getCodigo());
                table.addCell(pedidoMedicamento.getFechaPedido().toString());
                table.addCell(pedidoMedicamento.getEstado());
                table.addCell(pedidoMedicamento.getCantidad().toString());
                table.addCell(pedidoMedicamento.getTotal().toString());

                // Actualizar contadores y acumuladores
                switch (pedidoMedicamento.getEstado()) {
                    case "Recibido":
                        countRecibido++;
                        totalRecibido += pedidoMedicamento.getTotal().doubleValue();
                        break;
                    case "Rechazado":
                        countRechazado++;
                        totalRechazado += pedidoMedicamento.getTotal().doubleValue();
                        break;
                    case "Pendiente":
                        countPendiente++;
                        totalPendiente += pedidoMedicamento.getTotal().doubleValue();
                        break;
                }
            }
            // Agregar la tabla al documento
