 ...
 */
public class OrdenCobroPDF {
/**
 * Método que permite generar un PDF con la información de una orden de cobro
 * @param ordenCobro
 * @return
 * @throws IOException
 */
    public ByteArrayOutputStream generarPdfOrdenCobro(OrdenCobro ordenCobro) throws IOException {
        ...
        try {
            ...
            Paragraph title = new Paragraph("Orden de Cobro", titleFont);
            ...
            addRow(table, "Fecha de Emisión", ordenCobro.getFechaEmision().toString(), cyanFont, whiteFont, cyanColor);
            addRow(table, "Código", ordenCobro.getCodigo(), cyanFont, whiteFont, cyanColor);
            addRow(table, "Fecha Límite de Pago", ordenCobro.getFechaLimitePago().toString(), cyanFont, whiteFont,
                    cyanColor);
            addRow(table, "Estado", ordenCobro.getEstado(), cyanFont, whiteFont, cyanColor);
            addRow(table, "Monto", ordenCobro.getMonto().toString(), cyanFont, whiteFont, cyanColor);
            addRow(table, "Nombre Auditor Emisor", ordenCobro.getNombreAuiditorEmisor(), cyanFont, whiteFont,
                    cyanColor);
            addRow(table, "Cédula Auditor Emisor", ordenCobro.getCedulaAuditorEmisor(), cyanFont, whiteFont, cyanColor);

            // Agregar la tabla al documento
            document.add(table);

            // Añadir texto al final de la tabla
            Paragraph finalText = new Paragraph("Esta orden solo sera valida hasta: " + ordenCobro.getFechaLimitePago(),
                    whiteFont);
            ...
            Paragraph finalText2 = new Paragraph("De no pagarla perderá su agendamiento de cita", redFont);
